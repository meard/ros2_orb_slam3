...
MonocularMode::MonocularMode() : Node("mono_node_cpp")
{
    ...
    pubcamPoseName = "/mono_py_driver/cam_pose";
    ...
    //* publisher to send out camera pose for vio navigation
    camPose_publisher_ = this->create_publisher<geometry_msgs::msg::Pose>(pubcamPoseName, 10);
...
void MonocularMode::Img_callback(const sensor_msgs::msg::Image &msg)
{
    ...
    std::cout << "Sophus Tcw Pose: \n"
              << Tcw.matrix() << "\n"
              << std::endl;
    // camPose_publisher_->publish(message);

    Sophus::SE3f Tcw2 = Tcw.inverse();
    std::cout << "Sophus Tcw Inverse Pose: \n"
              << Tcw2.matrix() << "\n"
              << std::endl;

    // Sophus::SE3f Tcw3 = Tcw.rotation();
    // std::cout << "Sophus Tcw Rotation: \n" << Tcw3.matrix() << "\n" << std::endl;

    // Sophus::SE3f Tcw4 = Tcw.translation();
    // std::cout << "Sophus Tcw Translation: \n" << Tcw4.matrix() << "\n" << std::endl;
}
